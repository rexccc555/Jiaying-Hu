...
# Take a Day Off 站内页可跨域调用本机助手（仅本地 HTTP 场景；线上 HTTPS 需打开本机页）
CORS_ORIGINS = [
    "http://127.0.0.1:3000",
    "http://localhost:3000",
    "http://127.0.0.1:1780",
    "http://localhost:1780",
    "https://takeadayoff.co.nz",
    "https://www.takeadayoff.co.nz",
]


def create_app() -> FastAPI:
    ensure_data_dirs()
    app = FastAPI(title="Take a Day Off · CutPost", version=__version__)
    app.add_middleware(
        CORSMiddleware,
...
def serve(host: str = "127.0.0.1", port: int = DEFAULT_PORT, open_browser: bool = True) -> None:
    ...
    open_url = os.environ.get("CUTPOST_OPEN_URL") or url
    if probe_health(host, port):
        print(f"本机助手已在运行：{url}")
        if open_browser:
    ...
    print(f"本机助手：{url}")
    print("Take a Day Off 站内页：/[locale]/xhs （需本助手在跑）")
    print("默认只填表，不会偷偷发布。")
